  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("視覺化與結果 (Visualizations & Results)",
                 fluidRow(
                   column(6, h5("Probability Distribution (Fixed \\(\\theta\\))"), plotOutput("prob_plot")),
                   column(6, h5("Likelihood Function (Fixed \\(x\\))"), plotOutput("like_plot"))
                 ),
                 fluidRow(
